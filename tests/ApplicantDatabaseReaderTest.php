...
class ApplicantDatabaseReaderTest extends TestCase
{
    ...
    public function testOverduePaymentQueryWithoutWeek() {
        ...
        $this->assertNotEmpty($sql);
        $this->assertEquals("SELECT a.* from `applicants` a WHERE now() > DATE_ADD(a.paymentmailed, INTERVAL 2 WEEK) AND a.paymentreceived IS NOT NULL AND statusId != (select id from status where name in ('PAID'))", trim($sql));
    }
    ...
    public function testOverduePaymentQueryWithWeek() {
        ...
        $this->assertNotEmpty($sql);
        $this->assertEquals("SELECT a.* from `applicants` a WHERE now() > DATE_ADD(a.paymentmailed, INTERVAL 2 WEEK) AND a.week LIKE '%AWeek%' AND a.paymentreceived IS NOT NULL AND statusId != (select id from status where name in ('PAID'))", trim($sql));
    }
